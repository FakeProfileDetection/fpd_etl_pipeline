...
from scripts.utils.version_manager_v2 import VersionManagerV2, migrate_from_v1
...
@click.option('--include-uploaded', is_flag=True, help='Also clean uploaded versions (dangerous!)')
def cleanup(dry_run: bool, keep_count: int, include_uploaded: bool):
    ...
    for v in to_delete[:10]:  # Show first 10
        age_days = (datetime.now() - datetime.fromisoformat(v["created_at"].replace('Z', '+00:00'))).days
    ...
    if vm.data["current"] in [v["version_id"] for v in to_delete]:
        vm.data["current"] = remaining_versions[0]["version_id"] if remaining_versions else None
...
def stats():
    ...
    uploaded = sum(1 for v in versions if v.get("summary", {}).get("artifacts_uploaded"))
