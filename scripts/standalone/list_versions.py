...
@click.option('--uploaded-only', is_flag=True, help='Show only versions with uploaded artifacts')
@click.option('--json', 'output_json', is_flag=True, help='Output as JSON')
def main(limit: int, show_all: bool, complete_only: bool, uploaded_only: bool, output_json: bool):
    ...
    for v in filtered_versions:
        if (v.get('status') == 'complete' and 
            v.get('summary', {}).get('artifacts_uploaded') and 
            not latest_complete_uploaded):
            latest_complete_uploaded = v['version_id']
    ...
    for i, version in enumerate(filtered_versions):
        ...
        completed_stages = [name for name, info in stages.items() if info.get('completed')]
    ...
    if latest_complete_uploaded:
        click.echo(f"   Download latest: python scripts/standalone/download_artifacts.py --version-id {latest_complete_uploaded}")
    click.echo("   Show all versions: python scripts/standalone/list_versions.py --all")
    click.echo("   Show only uploaded: python scripts/standalone/list_versions.py --uploaded-only")
