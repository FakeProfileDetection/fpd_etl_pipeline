...
def create_tarball(version_id: str, artifacts_dir: Path, include_pii: bool = False) -> Optional[Path]:
...
@click.option('--force', is_flag=True,
              help='Force re-upload even if version already exists')
def main(version_id: str, include_pii: bool, dry_run: bool, force: bool):
    ...
    total_size_mb = sum(f.stat().st_size for f in version_dir.rglob("*") if f.is_file()) / 1024 / 1024
    ...
    if dry_run:
        ...
        if not include_pii:
            ...
            excluded = 0
            for pattern in ["demographics.json", "consent.json", "metadata.csv", "email", "start_time.json"]:
