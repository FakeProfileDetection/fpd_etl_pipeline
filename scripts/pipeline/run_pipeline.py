...
from scripts.utils.logger_config import get_pipeline_logger, setup_pipeline_logging
...
class Pipeline:
    ...
    def __init__(
        ...
        if config.get("UPLOAD_ARTIFACTS"):
            self.artifact_manager = CloudArtifactManager(
                version_id=version_id, bucket_name=config.get("BUCKET_NAME")
            )
        else:
    ...
    def run_stage(self, stage_name: str, stage_func: callable, **kwargs) -> bool:
        ...
        logger.info(
            f"{'[DRY RUN] ' if self.dry_run else ''}Running stage: {stage_name}"
        )
        ...
        try:
            ...
            self.version_manager.update_stage_info(
                self.version_id, stage_name, stage_info
            )
            ...
            if version_info and "stages" in version_info:
                ...
                if (
                    ...
                    and "clean_data" in version_info["stages"]
                ):
                ...
                # Get download size from download_data stage
                    ...
                    stage_name == "download_data"
                    and "download_data" in version_info["stages"]
                ):
                    # Read from download metadata if available
                    artifacts_dir = (
                        Path(self.config.get("ARTIFACTS_DIR", "artifacts"))
                        / self.version_id
                    )
                    download_log = (
                        artifacts_dir
                    ...
                    if download_log.exists():
                        try:
                            ...
                            with open(download_log) as f:
                                ...
                                self.pipeline_stats["data_downloaded_mb"] = (
            ...
            self.version_manager.update_stage_info(
                self.version_id, stage_name, {"completed": False, "error": str(e)}
            )
            return False
    ...
    def _check_dependencies(self, stage: str) -> bool:
        ...
        dependencies = {
            ...
            "top_il": ["extract_features"],  # Top IL depends on features
    ...
    def create_and_upload_tarball(self) -> bool:
        ...
        try:
            ...
            with tarfile.open(tarball_path, "w:gz") as tar:
                tar.add(
                    version_dir,
                    arcname=self.version_id,
                    filter=lambda x: None if should_exclude(x) else x,
                )

            logger.info(
                f"Archive created: {tarball_path.stat().st_size / 1024 / 1024:.2f} MB"
            )
            ...
            if result.returncode == 0:
                ...
                logger.info(
                    "Per policy: Only additions are allowed to existing versions"
                )
                logger.info(
                    "If you need to modify existing artifacts, create a new version"
                )
                return False
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "stage_times": {
                k: round(v, 2) for k, v in self.pipeline_stats["stage_times"].items()
            },
        }
...
@click.option(
    "--mode",
    type=click.Choice(["full", "incr", "force"]),
    default="incr",
    help="Pipeline run mode",
)
@click.option(
    "--stages", "-s", multiple=True, help="Specific stages to run (default: all)"
)
    ...
    if not version_id:
        if mode == "full":
            ...
            version_mgr.register_version(
                version_id, {"mode": mode, "stages": list(stages)}
            )
    ...
    if not stages:
        if mode == "full":
            ...
            stage_name_mapping = {
                ...
                "extract_top_il_features": "top_il",
                "run_eda": "eda",
            }
            ...
            if not stages_to_run:
                click.echo(
                    "ℹ️  All stages already completed. Use --mode force to re-run."
                )
                stages = []
            else:
                stages = stages_to_run
                click.echo(
                    f"📋 Incremental mode - running incomplete stages: {', '.join(stages)}"
                )
        elif mode == "force":
            ...
            if not version_id:
                ...
                if not version_id:
                    click.echo(
                        "❌ No current version found. Use --mode full for new data."
                    )
                    return
    ...
    if success:
        ...
        if upload_artifacts and not dry_run:
            ...
            if pipeline.create_and_upload_tarball():
                ...
                logger.info(
                    "You can retry with: python scripts/standalone/upload_artifacts.py --version-id "
                    + version_id
                )
        elif not upload_artifacts:
            logger.info("\n💡 Tip: To upload artifacts after review, run:")
            logger.info(
                f"    python scripts/standalone/upload_artifacts.py --version-id {version_id}"
            )
    else:
        logger.error("\n❌ Pipeline failed!")
