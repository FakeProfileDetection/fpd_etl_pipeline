...
class ExtractKeypairsStage:
    ...
    def normalize_key(self, key: str) -> str:
        """
        Normalize key values to ensure consistency:
        - Convert single uppercase letters to lowercase (A -> a)
        - Keep special keys as-is (Key.shift, Key.space, etc.)
        """
        if isinstance(key, str) and len(key) == 1 and key.isupper():
            return key.lower()
        return key
    ...
    def parse_raw_file(self, filepath: Path) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if len(df.columns) != 3:
                logger.warning(
                    f"Invalid format in {filepath.name}: wrong number of columns"
                )
                return None
            ...
            if not df["type"].isin(["P", "R"]).all():
                logger.warning(
                    f"Invalid format in {filepath.name}: unexpected press types"
                )
                return None

            # Normalize keys (convert single uppercase letters to lowercase)
            df["key"] = df["key"].apply(self.normalize_key)
            ...
            self.stats["processing_errors"].append(
                f"Read error in {filepath.name}: {e!s}"
            )
            return None
    ...
    def match_press_release_pairs(self, df: pd.DataFrame) -> List[KeystrokeEvent]:
        ...
        for idx, row in df.iterrows():
            ...
            if event_type == "P":
                ...
                key_stacks[key].append(
                    {"key": key, "press_time": timestamp, "press_idx": idx}
                )
        ...
        events.sort(
            key=lambda x: x.press_time if x.press_time is not None else x.release_time
        )
    ...
    def calculate_features(
        ...
        features = KeypairFeatures(
            ...
            key1_timestamp=key1.press_time
            if key1.press_time is not None
            else key1.release_time,
        )
        ...
        if not key2.valid:
            ...
            if features.error_description == "No error":
                features.error_description = self.ERROR_TYPES.get(
                    key2.error, key2.error
                ).replace("key1", "key2")
            else:
        ...
        if key1.valid and key2.valid:
            try:
                ...
                features.valid = False
                features.error_description = f"Calculation error: {e!s}"
    ...
    def extract_features_from_file(self, filepath: Path) -> List[KeypairFeatures]:
        ...
        for i in range(len(events) - 1):
            features = self.calculate_features(
                events[i], events[i + 1], user_id, platform_id, video_id, session_id, i
            )
    ...
    def process_user_directory(self, user_dir: Path) -> pd.DataFrame:
        ...
        for csv_file in sorted(csv_files):
            ...
            try:
                ...
                self.stats["processing_errors"].append(
                    f"Process error in {csv_file.name}: {e!s}"
                )
        ...
        if all_features:
            ...
            if len(df) > 0 and df["valid"].any():
                ...
                df.loc[
                    valid_mask & (df["IL"].abs() > 1000), "outlier"
                ] = True  # |IL| > 1 second
    ...
    def run(self, input_dir: Path) -> Path:
        ...
        if all_keypair_data:
            ...
            self.stats["invalid_keypairs"] = (
                self.stats["total_keypairs"] - self.stats["valid_keypairs"]
            )
        else:
        ...
        if not self.dry_run:
            ...
            extraction_stats = {
                ...
                "summary": {
                    ...
                    "validity_rate": float(
                        (
                            self.stats["valid_keypairs"]
                            / self.stats["total_keypairs"]
                            * 100
                        )
                        if self.stats["total_keypairs"] > 0
                        else 0
                    ),
                },
                "error_distribution": {
                    k: int(v) for k, v in self.stats["error_counts"].items()
                },
        ...
        logger.info(
            f"  Valid keypairs: {self.stats['valid_keypairs']} "
