...
def run(version_id: str, 
        config: Dict[str, Any], 
        artifact_manager: Optional['CloudArtifactManager'] = None,
        **kwargs) -> Path:
    """
    Run the [STAGE_NAME] stage of the pipeline.
    ...
    logger.info(f"Starting [STAGE_NAME] stage for version {version_id}")
    start_time = datetime.now()
    
    # Setup paths
    # TODO: Update these based on your stage
    input_dir = Path(config["PREV_STAGE_DIR"].format(version_id=version_id))
    output_dir = Path(config["THIS_STAGE_DIR"].format(version_id=version_id))
    ...
    try:
        ...
        if artifact_manager and config.get("UPLOAD_ARTIFACTS"):
            ...
            #     stage="[STAGE_NAME]",
            #     description="Processed data from [STAGE_NAME] stage",
        ...
        logger.info(f"✅ [STAGE_NAME] stage completed in {metadata['duration_seconds']:.1f}s")
        return output_dir
