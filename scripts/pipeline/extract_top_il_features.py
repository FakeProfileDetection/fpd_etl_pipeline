...
class ExtractTopILFeaturesStage:
    ...
    def extract_top_features(self, df: pd.DataFrame, top_features: List[str], 
        ...
        logger.info(f"IL features retained: {len([c for c in existing_columns if c.startswith('IL_')])}")
    ...
    def process_feature_type(self, input_dir: Path, keypairs_dir: Path, feature_type: str, 
        ...
        if not feature_file.exists():
            # Try parquet format
            feature_file = input_dir / f"statistical_{feature_type}" / "features.parquet"
            if not feature_file.exists():
    ...
    def run(self, k: int = 10) -> Path:
        ...
        if not input_dir.exists():
            ...
            if old_input_dir.exists():
                ...
                raise FileNotFoundError(f"Statistical features directory not found: {input_dir}")
        ...
        logger.info(f"  Feature types processed: {len(self.stats['features_processed'])}")
